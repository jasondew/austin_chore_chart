...
decodeState =
  ...
  in
    ("completed_chores" := Json.list completedChore)
...
unpaidChoresTable address completedChores =
  let
    unpaidChores = List.filter unpaid completedChores
  in
    Html.div
      [ class "panel panel-default panel-primary" ]
...
totalRow address completedChores =
  let
    total = List.sum <| List.map .rate <| List.map .chore completedChores
  in
    Html.tr
      []
...
completedChoreRow completedChore =
  Html.tr
    [ Attributes.key <| toString completedChore.id ]
